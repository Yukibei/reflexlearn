...
这条通道原先只承载短问候，够用；FS-18 接入学术问答后回答带代码块、公式与有序列表，
...
- **牛牛面板疑似跳转 `/chat`**：用户报告「主页点击小牛发送信息会被直接跳转到 /chat」，
  但排查未复现——`CompanionPanel` 自带输入框与对话列表、发送走 `/tutor/ask`，
  companion 目录无任何 router.push；首页与 /today 的 `/chat` 入口都是明确按钮而非伪装输入框。
  **需要用户指认具体页面与元素后再修**，不要盲改。
- **历史对话缺失**：`useChat` 的会话只活在页面内存，刷新即失。后端 `session_id` 与 Redis
  多轮记忆已具备，缺的是「会话列表 + 历史读取」接口与界面。用户已确认优先级排在管家之后。
